  ...
  class contents extends memberLog{
    ...
    function __construct(){
      ...
      if(isset($_POST['mode'])){
        ...
        }else if($this->mode == 'onlyComment'){
            $this->productCommentsLoad($_POST['productID']);
        }
      }
    }
    
    function productCommentsLoad($productID){
         //댓글을 반환하는 메소드 호출함, 알규먼트로 게시물 번호 전달
         $commentLoad = $this->commentLoad($productID);
